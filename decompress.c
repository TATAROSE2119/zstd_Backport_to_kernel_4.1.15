...
#define ZSTD_MAXWINDOWSIZE_DEFAULT                                             \
        ((1 << ZSTD_WINDOWLOG_MAX) + 1) /* defined within zstd.h */
#endif
...
typedef struct {
        ...
        HUF_DTable hufTable[HUF_DTABLE_SIZE(
                HufLog)]; /* can accommodate HUF_decompress4X */
...
struct ZSTD_DCtx_s {
        ...
        const void *vBase;   /* virtual start of previous segment if it was just
                                before curr one */
        const void *dictEnd; /* end of previous segment */
        size_t expected;
        ZSTD_frameParams fParams;
        blockType_e bType; /* used in ZSTD_decompressContinue(), to transfer
                              blockType between header decoding and block
                              decoding stages */
        ZSTD_dStage stage;
        U32 litEntropy;
        U32 fseEntropy;
...
/**
 * @brief 计算解压缩上下文所需的工作空间大小
 * 
 * 该函数返回 ZSTD 解压缩上下文所需的总工作空间大小（以字节为单位）。
 * 工作空间包括 ZSTD_stack 结构和 ZSTD_DCtx 结构的内存需求，
 * 两者都按 ZSTD 对齐要求进行对齐。
 * 
 * @return size_t 返回对齐后的 ZSTD_stack 和 ZSTD_DCtx 结构大小之和
 */
...
size_t ZSTD_decompressBegin(ZSTD_DCtx *dctx) {
        ...
        memcpy(dctx->entropy.rep, repStartValue,
...
void ZSTD_copyDCtx(ZSTD_DCtx *dstDCtx, const ZSTD_DCtx *srcDCtx) {
        size_t const workSpaceSize =
                (ZSTD_BLOCKSIZE_ABSOLUTEMAX + WILDCOPY_OVERLENGTH) +
                ZSTD_frameHeaderSize_max;
        memcpy(dstDCtx, srcDCtx,
               sizeof(ZSTD_DCtx) -
                       workSpaceSize); /* no need to copy workspace */
}
...
size_t ZSTD_getFrameParams(ZSTD_frameParams *fparamsPtr, const void *src,
                ...
                if ((ZSTD_readLE32(src) & 0xFFFFFFF0U) ==
                    ZSTD_MAGIC_SKIPPABLE_START) {
                        ...
                        fparamsPtr->frameContentSize =
                                ZSTD_readLE32((const char *)src + 4);
...

        {
                ...
                if ((fhdByte & 0x08) != 0)
                        return ERROR(
                                frameParameter_unsupported); /* reserved bits,
                                                                which must be
                                                                zero */
                if (!singleSegment) {
                        ...
                        if (windowLog > ZSTD_WINDOWLOG_MAX)
                                return ERROR(
                                        frameParameter_windowTooLarge); /* avoids
                                                                           issue
                                                                           with
                                                                           1 <<
                                                                           windowLog
                ...
                case 0:
                        ...
                        break;
                case 1:
                        frameContentSize = ZSTD_readLE16(ip + pos) + 256;
                        break;
                case 2:
                        frameContentSize = ZSTD_readLE32(ip + pos);
                        break;
                case 3:
                        frameContentSize = ZSTD_readLE64(ip + pos);
                        break;
                }
                if (!windowSize)
                        ...
                        if ((magicNumber & 0xFFFFFFF0U) ==
                            ZSTD_MAGIC_SKIPPABLE_START) {
                                ...
                                if (srcSize < ZSTD_skippableHeaderSize)
                                        return ERROR(srcSize_wrong);
                                skippableSize =
                                        ZSTD_readLE32((const BYTE *)src + 4) +
...
static size_t ZSTD_decodeFrameHeader(ZSTD_DCtx *dctx, const void *src,
                                     size_t headerSize) {
        size_t const result =
                ZSTD_getFrameParams(&(dctx->fParams), src, headerSize);
...
static size_t ZSTD_copyRawBlock(void *dst, size_t dstCapacity, const void *src,
                                size_t srcSize) {
...
static size_t ZSTD_setRleBlock(void *dst, size_t dstCapacity, const void *src,
                               size_t srcSize, size_t regenSize) {
        if (srcSize != 1)
                return ERROR(srcSize_wrong);
...

        {
                ...
                case set_basic: {
                        ...
                        if (lhSize + litSize + WILDCOPY_OVERLENGTH >
                            srcSize) { /* risk reading beyond src buffer with
                                          wildcopy */
                ...
                case set_rle: {
                        ...
                        if (litSize > ZSTD_BLOCKSIZE_ABSOLUTEMAX)
                                return ERROR(corruption_detected);
                        memset(dctx->litBuffer, istart[lhSize],
...
/*! ZSTD_buildSeqTable() :
   ...
   ZSTD_isError()
*/
static size_t ZSTD_buildSeqTable(FSE_DTable *DTableSpace,
        ...
        case set_compressed: {
                ...
                {
                        size_t const headerSize = FSE_readNCount(
                                norm, &max, &tableLog, src, srcSize);
                ...
                return ERROR(srcSize_wrong); /* minimum possible size */
        {
                ...
                /* Build DTables */
                {
                        size_t const llhSize = ZSTD_buildSeqTable(
                                dctx->entropy.LLTable, &dctx->LLTptr, LLtype,
                                MaxLL, LLFSELog, ip, iend - ip,
                                LL_defaultDTable, dctx->fseEntropy,
                                dctx->entropy.workspace,
                ...
                }
                {
                        size_t const ofhSize = ZSTD_buildSeqTable(
                                dctx->entropy.OFTable, &dctx->OFTptr, OFtype,
                                MaxOff, OffFSELog, ip, iend - ip,
                ...
                }
                {
                        size_t const mlhSize = ZSTD_buildSeqTable(
                                dctx->entropy.MLTable, &dctx->MLTptr, MLtype,
                                MaxML, MLFSELog, ip, iend - ip,
...
size_t ZSTD_execSequenceLast7(BYTE *op, BYTE *const oend, seq_t sequence,
        ...
        BYTE *const oMatchEnd =
                op +
                sequenceLength; /* risk : address space overflow (32-bits) */
        ...
        if (oMatchEnd > oend)
                return ERROR(
                        dstSize_tooSmall); /* last match must start at a minimum
                                              distance of WILDCOPY_OVERLENGTH
                                              from oend */
        if (iLitEnd > litLimit)
                return ERROR(
                        corruption_detected); /* over-read beyond lit buffer */
        if (oLitEnd <= oend_w)
...
static seq_t ZSTD_decodeSequence(seqState_t *seqState) {
        ...
        seq.matchLength =
                ML_base[mlCode] +
                ((mlCode > 31) ? BIT_readBitsFast(&seqState->DStream, mlBits)
                               : 0); /* <=  16 bits */
        ...
        seq.litLength =
                LL_base[llCode] +
                ((llCode > 15) ? BIT_readBitsFast(&seqState->DStream, llBits)
                               : 0); /* <=  16 bits */
        if (ZSTD_32bits() ||
        ...
        /* ANS state update */
        FSE_updateState(&seqState->stateLL,
                        &seqState->DStream); /* <=  9 bits */
        FSE_updateState(&seqState->stateML,
                        &seqState->DStream); /* <=  9 bits */
        if (ZSTD_32bits())
                BIT_reloadDStream(&seqState->DStream); /* <= 18 bits */
        FSE_updateState(&seqState->stateOffb,
                        &seqState->DStream); /* <=  8 bits */
...
size_t ZSTD_execSequence(BYTE *op, BYTE *const oend, seq_t sequence,
        ...
        if (sequence.litLength > 8)
                ZSTD_wildcopy(op + 8, (*litPtr) + 8,
                              sequence.litLength -
                                      8); /* note : since oLitEnd <=
                                             oend-WILDCOPY_OVERLENGTH, no risk
        ...
        *litPtr = iLitEnd; /* update for next sequence */
        ...
        if (sequence.offset < 8) {
                ...
                static const U32 dec32table[] = {0, 1, 2, 1,
                                                 4, 4, 4, 4}; /* added */
                static const int dec64table[] = {8, 8, 8,  7,
                                                 8, 9, 10, 11}; /* subtracted */
        ...
        if (oMatchEnd > oend - (16 - MINMATCH)) {
                ...
                ZSTD_wildcopy(op, match,
                              (ptrdiff_t)sequence.matchLength -
...
static size_t ZSTD_decompressSequences(ZSTD_DCtx *dctx, void *dst,
        ...
        {
                size_t const seqHSize =
                        ZSTD_decodeSeqHeaders(dctx, &nbSeq, ip, seqSize);
                       ...
                       nbSeq;) {
                        nbSeq--;
                        {
                                ...
                                size_t const oneSeqSize = ZSTD_execSequence(
                                        op, oend, sequence, &litPtr, litEnd,
                                        base, vBase, dictEnd);
...
FORCE_INLINE seq_t ZSTD_decodeSequenceLong_generic(seqState_t *seqState,
        ...
        /* sequence */
        {
                ...
                if (!ofCode)
                        ...
                        if (longOffsets) {
                                int const extraBits =
                                        ofBits -
                                        MIN(ofBits, STREAM_ACCUMULATOR_MIN);
                                offset = OF_base[ofCode] +
...

        {
                ...
                seq.match = seqState->base + pos -
                            seq.offset; /* single memory segment */
                if (seq.offset > pos)
...
#define ADVANCED_SEQS 4
                ...
                seqState.gotoDict =
                        (uPtrDiff)dictEnd -
                        (uPtrDiff)base; /* cast to avoid undefined behaviour */
                CHECK_E(BIT_initDStream(&seqState.DStream, ip, iend - ip),
                        corruption_detected);
                FSE_initDState(&seqState.stateLL, &seqState.DStream,
                               dctx->LLTptr);
                     ...
                     seqNb++) {
                        ...
                        size_t const oneSeqSize = ZSTD_execSequenceLong(
                                ...
                                sequences[(seqNb - ADVANCED_SEQS) &
                                          STOSEQ_MASK],
                                &litPtr, litEnd, base, vBase, dictEnd);
                        if (ZSTD_isError(oneSeqSize))
                                return oneSeqSize;
                ...
                for (; seqNb < nbSeq; seqNb++) {
                        size_t const oneSeqSize = ZSTD_execSequenceLong(
                                op, oend, sequences[seqNb & STOSEQ_MASK],
                                &litPtr, litEnd, base, vBase, dictEnd);
                        if (ZSTD_isError(oneSeqSize))
                                return oneSeqSize;
                        op += oneSeqSize;
...
static size_t ZSTD_decompressBlock_internal(
        ZSTD_DCtx *dctx, void *dst, size_t dstCapacity, const void *src,
        ...
        /* Decode literals section */
        {
                size_t const litCSize =
                        ZSTD_decodeLiteralsBlock(dctx, src, srcSize);
        ...
        if (sizeof(size_t) > 4) /* do not enable prefetching on 32-bits x86, as
                ...
                /* if that's the correct cause, then 32-bits ARM should be
                 * affected differently */
                /* it would be good to test this on ARM real hardware, to see if
                 * prefetch version improves speed */
...
static void ZSTD_checkContinuity(ZSTD_DCtx *dctx, const void *dst) {
        if (dst != dctx->previousDstEnd) { /* not contiguous */
                ...
                dctx->vBase = (const char *)dst -
                              ((const char *)(dctx->previousDstEnd) -
...
size_t ZSTD_generateNxBytes(void *dst, size_t dstCapacity, BYTE byte,
                            size_t length) {
 ...
 * skippable frame `srcSize` must be at least as large as the frame contained
        ...
        if (srcSize >= ZSTD_skippableHeaderSize &&
            (ZSTD_readLE32(src) & 0xFFFFFFF0U) == ZSTD_MAGIC_SKIPPABLE_START) {
                ...
                size_t const headerSize =
                        ZSTD_frameHeaderSize(ip, remainingSize);
                ...
                while (1) {
                        ...
                        size_t const cBlockSize = ZSTD_getcBlockSize(
                                ip, remainingSize, &blockProperties);
...
static size_t ZSTD_decompressFrame(ZSTD_DCtx *dctx, void *dst,
                                   size_t dstCapacity, const void **srcPtr,
                                   size_t *srcSizePtr) {
        ...
        /* Frame Header */
        {
                size_t const frameHeaderSize =
                        ZSTD_frameHeaderSize(ip, ZSTD_frameHeaderSize_prefix);
        ...
        while (1) {
                ...
                case bt_compressed:
                        decodedSize = ZSTD_decompressBlock_internal(
                                dctx, op, oend - op, ip, cBlockSize);
                        break;
                case bt_raw:
                        decodedSize = ZSTD_copyRawBlock(op, oend - op, ip,
                                                        cBlockSize);
                        break;
                case bt_rle:
                        decodedSize = ZSTD_generateNxBytes(
                                op, oend - op, *ip, blockProperties.origSize);
                        break;
                case bt_reserved:
                default:
                        return ERROR(corruption_detected);
        ...
        if (dctx->fParams
                    .checksumFlag) { /* Frame content checksum verification */
...
static size_t ZSTD_decompressMultiFrame(ZSTD_DCtx *dctx, void *dst,
        ...
        while (srcSize >= ZSTD_frameHeaderSize_prefix) {
                ...
                if (ddict) {
                         ...
                         * == NULL, so use this in all cases but ddict */
                ...
                {
                        const size_t res = ZSTD_decompressFrame(
                                dctx, dst, dstCapacity, &src, &srcSize);
                        if (ZSTD_isError(res))
                                return res;
                        /* don't need to bounds check this, ZSTD_decompressFrame
                         * will have already */
                        dst = (BYTE *)dst + res;
                        dstCapacity -= res;
                }
...
size_t ZSTD_decompressContinue(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity,
        ...
        case ZSTDds_getFrameHeaderSize:
                ...
                if ((ZSTD_readLE32(src) & 0xFFFFFFF0U) ==
                    ZSTD_MAGIC_SKIPPABLE_START) { /* skippable frame */
                        ...
                        dctx->expected =
                                ZSTD_skippableHeaderSize -
                                ZSTD_frameHeaderSize_prefix; /* magic number +
                                                                skippable frame
                                                                length */
                ...
                dctx->headerSize =
                        ZSTD_frameHeaderSize(src, ZSTD_frameHeaderSize_prefix);
                if (ZSTD_isError(dctx->headerSize))
                        return dctx->headerSize;
                memcpy(dctx->headerBuffer, src, ZSTD_frameHeaderSize_prefix);
                if (dctx->headerSize > ZSTD_frameHeaderSize_prefix) {
                        dctx->expected =
                                dctx->headerSize - ZSTD_frameHeaderSize_prefix;
        ...
        case ZSTDds_decodeBlockHeader: {
                ...
                size_t const cBlockSize =
                        ZSTD_getcBlockSize(src, ZSTD_blockHeaderSize, &bp);
                if (ZSTD_isError(cBlockSize))
                        return cBlockSize;
        ...
        case ZSTDds_decompressBlock: {
                ...
                case bt_compressed:
                        ...
                        break;
                case bt_raw:
                        rSize = ZSTD_copyRawBlock(dst, dstCapacity, src,
                                                  srcSize);
                        break;
                case bt_rle:
                        rSize = ZSTD_setRleBlock(dst, dstCapacity, src, srcSize,
                                                 dctx->rleSize);
                        break;
                ...
                if (dctx->stage ==
                    ZSTDds_decompressLastBlock) {         /* end of frame */
                        if (dctx->fParams.checksumFlag) { /* another round for
                                                             frame checksum */
                                dctx->expected = 4;
                                dctx->stage = ZSTDds_checkChecksum;
                        } else {
        ...
        case ZSTDds_checkChecksum: {
                ...
                U32 const check32 = ZSTD_readLE32(
                        src); /* srcSize == 4, guaranteed by dctx->expected */
                if (check32 != h32)
                        return ERROR(checksum_wrong);
                dctx->expected = 0;
                dctx->stage = ZSTDds_getFrameHeaderSize;
...
static size_t ZSTD_refDictContent(ZSTD_DCtx *dctx, const void *dict,
                                  size_t dictSize) {
        dctx->dictEnd = dctx->previousDstEnd;
        dctx->vBase =
...
static size_t ZSTD_loadEntropy(ZSTD_entropyTables_t *entropy,
...

        {
                ...
                CHECK_E(FSE_buildDTable_wksp(
                                entropy->MLTable, matchlengthNCount,
...
static size_t ZSTD_decompress_insertDictionary(ZSTD_DCtx *dctx,
                ...
                return ZSTD_refDictContent(dctx, dict, dictSize);
        {
                ...
                if (magic != ZSTD_DICT_MAGIC) {
                        return ZSTD_refDictContent(
                                dctx, dict, dictSize); /* pure content mode */
                }
        }
        ...
        {
                size_t const eSize =
                        ZSTD_loadEntropy(&dctx->entropy, dict, dictSize);
...
static void ZSTD_refDDict(ZSTD_DCtx *dstDCtx, const ZSTD_DDict *ddict) {
        ...
        if (ddict) {                   /* support refDDict on NULL */
                ...
                dstDCtx->dictEnd =
                        (const BYTE *)ddict->dictContent + ddict->dictSize;
...
static ZSTD_DDict *ZSTD_createDDict_advanced(const void *dict, size_t dictSize,
        ...
        {
                ZSTD_DDict *const ddict = (ZSTD_DDict *)ZSTD_malloc(
                        sizeof(ZSTD_DDict), customMem);
                if (!ddict)
                        return NULL;
                ...
                ddict->entropy.hufTable[0] =
                        (HUF_DTable)((HufLog) * 0x1000001); /* cover both little
                                                               and big endian */
...
ZSTD_DDict *ZSTD_initDDict(const void *dict, size_t dictSize, void *workspace,
                           size_t workspaceSize) {
        ZSTD_customMem const stackMem =
                ZSTD_initStack(workspace, workspaceSize);
 ...
 *  - The frame was built with dictID intentionally removed. Whatever dictionary
 * is necessary is a hidden information. Note : this use case also happens when
 * using a non-conformant dictionary.
...
size_t ZSTD_DStreamWorkspaceBound(size_t maxWindowSize) {
        ...
        size_t const outBuffSize =
                maxWindowSize + blockSize + WILDCOPY_OVERLENGTH * 2;
...
ZSTD_DStream *ZSTD_initDStream(size_t maxWindowSize, void *workspace,
...

        {
                ...
                zds->outBuff =
                        (char *)ZSTD_malloc(neededOutSize, zds->customMem);
...
size_t ZSTD_DStreamInSize(void) {
        return ZSTD_BLOCKSIZE_ABSOLUTEMAX + ZSTD_blockHeaderSize;
}
...
size_t ZSTD_decompressStream(ZSTD_DStream *zds, ZSTD_outBuffer *output,
        ...
        while (someMoreWork) {
                switch (zds->stage) {
                case zdss_init:
                        ZSTD_resetDStream(zds); /* transparent reset on starting
                                                   decoding a new frame */
                                                /* fall-through */

                case zdss_loadHeader: {
                        size_t const hSize = ZSTD_getFrameParams(
                                &zds->fParams, zds->headerBuffer, zds->lhSize);
                        if (ZSTD_isError(hSize))
                                return hSize;
                        if (hSize != 0) { /* need more input */
                                size_t const toLoad =
                                        hSize -
                                        zds->lhSize; /* if hSize!=0, hSize >
                                                        zds->lhSize */
                                if (toLoad >
                                    (size_t)(iend -
                                             ip)) { /* not enough input to load
                                                       full header */
                                        memcpy(zds->headerBuffer + zds->lhSize,
                                               ip, iend - ip);
                                        zds->lhSize += iend - ip;
                                        ...
                                        return (MAX(ZSTD_frameHeaderSize_min,
                                                    hSize) -
                                                zds->lhSize) +
                                               ZSTD_blockHeaderSize; /* remaining
                                                                        header
                                                                        bytes +
                                                                        next
                                                                        block
                        ...
                        if (zds->fParams.frameContentSize &&
                            zds->fParams
                                    .windowSize /* skippable frame if == 0 */
                            && (U64)(size_t)(oend - op) >=
                                ...
                                size_t const h1Size =
                                        ZSTD_nextSrcSizeToDecompress(
                                                zds->dctx); /* ==
                                                               ZSTD_frameHeaderSize_prefix
                                                             */
                                CHECK_F(ZSTD_decompressContinue(
                                        zds->dctx, NULL, 0, zds->headerBuffer,
                                        h1Size));
                                {
                                        size_t const h2Size =
                                                ZSTD_nextSrcSizeToDecompress(
                                        ...
                                        CHECK_F(ZSTD_decompressContinue(
                                                zds->dctx, NULL, 0,
                                                zds->headerBuffer + h1Size,
                                                h2Size));
                                }
                        }

                        zds->fParams.windowSize =
                                MAX(zds->fParams.windowSize,
                ...
                case zdss_load: {
                        ...
                        size_t const toLoad =
                                neededInSize -
                                zds->inPos; /* should always be <= remaining
                                               space within inBuff */
                        size_t loadedSize;
                        if (toLoad > zds->inBuffSize - zds->inPos)
                                return ERROR(
                                        corruption_detected); /* should never
                                                                 happen */
                ...
                case zdss_flush: {
                        ...
                        size_t const flushedSize = ZSTD_limitCopy(
                                op, oend - op, zds->outBuff + zds->outStart,
                                toFlushSize);
                        op += flushedSize;
        ...
        output->pos += (size_t)(op - ostart);
        {
                ...
                if (!nextSrcSizeHint) { /* frame fully decoded */
                        if (zds->outEnd ==
                            zds->outStart) { /* output fully flushed */
                                if (zds->hostageByte) {
                        ...
                        if (!zds->hostageByte) { /* output not fully flushed;
                                                    keep last byte as hostage;
                                                    will be released when all
                                                    output is flushed */
                                input->pos--;    /* note : pos > 0, otherwise,
